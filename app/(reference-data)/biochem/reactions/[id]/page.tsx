...
function getPrototypeAtomMapping(reactionId: string): ReactionAtomMapping | undefined {
    // Curated atom-flow overlays for the first five available reactions:
    // rxn00001, rxn00002, rxn00003, rxn00004, and rxn00006.
    // These are hand-crafted prototypes from known structures/equations until
    // canonical atom-map data is generated by a dedicated mapping pipeline.
    if (reactionId === 'rxn00001') {
        ...
        return {
            ...
            cpd00012: {
                1: '#2563eb', // P1
                5: '#f59e0b', // P2
                4: '#14b8a6', // bridging O between phosphates
            },
            // Phosphate appears with stoich 2; one card represents both copies.
            cpd00009: {
                1: '#2563eb', // representative P mapping
                4: '#14b8a6', // hydroxyl oxygen likely exchanged during hydrolysis
            },
            // Water and proton shown for completeness of hydrolysis context.
            cpd00001: { 0: '#ef4444' }, // H2O oxygen
            cpd00067: { 0: '#ef4444' }, // proton source/sink indicator
        };
    }
    if (reactionId === 'rxn00002') {
        // urea-1-carboxylate amidohydrolase
        return {
            // Allophanate: NC(=O)NC(=O)[O-]
            cpd00742: {
                1: '#8b5cf6', // carbonyl carbon A
                4: '#0ea5e9', // carbonyl carbon B
                0: '#22c55e', // amide N A
                3: '#f97316', // amide N B
            },
            // CO2: O=C=O (stoich 2)
            cpd00011: { 1: '#8b5cf6' }, // representative carboxyl carbon
            // NH3/NH4+: ammonium representation in database
            cpd00013: { 0: '#22c55e' }, // representative amine nitrogen
        };
    }
    if (reactionId === 'rxn00003') {
        // pyruvate:pyruvate acetaldehydetransferase (decarboxylating)
        return {
            // ALCTT: CC(=O)C(C)(O)C(=O)[O-]
            cpd00668: {
                0: '#0ea5e9', // methyl carbon
                1: '#14b8a6', // keto carbonyl carbon
                6: '#eab308', // carboxyl carbon
            },
            // Pyruvate: CC(=O)C(=O)[O-] (stoich 2)
            cpd00020: {
                0: '#0ea5e9',
                1: '#14b8a6',
                3: '#eab308',
            },
            // CO2 byproduct side of the equilibrium
            cpd00011: { 1: '#eab308' },
        };
    }
    if (reactionId === 'rxn00004') {
        // 4-hydroxy-4-methyl-2-oxoglutarate pyruvate-lyase
        return {
            // Parapyruvate: CC(O)(CC(=O)C(=O)[O-])C(=O)[O-]
            cpd02570: {
                0: '#8b5cf6', // methyl carbon
                1: '#0ea5e9', // tertiary center
                4: '#22c55e', // first carbonyl carbon in side chain
                6: '#f97316', // second carbonyl carbon in side chain
            },
            // Pyruvate product (stoich 2, one card shown)
            cpd00020: {
                0: '#8b5cf6',
                1: '#22c55e',
                3: '#f97316',
                    ...
                    {prototypeAtomMapping && (
                        <DetailRow label="Atom mapping overlay">
                            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                                Prototype mode: curated atom-flow overlays are enabled for the first
                                five available reactions (rxn00001, rxn00002, rxn00003, rxn00004,
                                rxn00006) to validate rendering behavior before canonical mapping
                                data is available.
                            </Typography>
                        </DetailRow>
                    )}
